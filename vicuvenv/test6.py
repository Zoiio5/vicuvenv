...
class VideoProcessor:
    ...
    def initialize_excel(self, file_path):
        ...
        self.libro = openpyxl.libro()
        self.cuadricula = self.libro.active
        self.cuadricula.append(["Frame", "Confidence", "Name", "Time"])
    ...
    def update_excel(self, frame_number, detections):
        ...
        for _, row in detections.iterrows():
            # Only append the 'name', 'confidence', and 'time' to the cuadricula
            self.cuadricula.append([row['name'], row['confidence'], time_str])
        self.libro.save(self.ruta_excel)
    ...
    def annotate_frame(self, frame, detections):
        ...
        labels = [
            f"#{tracker_id} {self.modelo.names[class_id]}"
    ...
    def process_next_video(self):
        if self.current_video_index < len(self.video_files):
            ...
            if self.processor.process_video(video_file):
                ...
                self.ruta_excel = os.path.join(self.excel_folder, excel_file_name)
                self.processor.initialize_excel(self.ruta_excel)
                self.frame_number = 0
                self.detections = []
                self.next_frame()
            else:
